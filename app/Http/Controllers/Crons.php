...
class Crons extends Controller
{
    ...
    protected function refuseMissedOrders($now){
        ...
        //get all new orders  for that day not previous day because we use time 
    ...
    protected function change_status_30_min($now){
        ...
        $date   = date('Y-m-d', strtotime("-1 days"));
        $orders = DB::table('orders_headers')->where('status_id',"=" , 2)   
            ->where(DB::raw('DATE(created_at)'), ">=", $date)
            ->select(DB::raw('TIME(provider_accept_order_date) AS created_time'), 'order_id', 'user_id', 'payment_type', 'total_value')
            ->get();
        if($orders->count()){
            foreach($orders AS $order){
                
                  // date  of accept order by provider 
                $created_at = strtotime($order->created_time);
                $diff = round(abs($now - $created_at) / 60,2);
